...
sub help {
	...
	Для создания домена(ов) используйте следующие команды:
	
	perl do-vh.pl имя_пользователя файл_списка_доменов noisp # без панели isp
	
	perl do-vh.pl имя_пользователя файл_списока_доменов isp # с учетом панели isp
	
	Файл содержит домены в таком виде:
	domain.tld
	domain2.tld
	и т.д.
